...
readFiles = "../documents/set2/doc"
writeFiles = "./write/set2/write"
...
def writeToBin(huffman_tree):
    ...
    for x in range(0,numDoc):
        filer = readFiles+str(x)+".txt"
        filew = writeFiles+str(x)+".bin"
        #print "writing to "+ filew
        #fdbug = "write/set"+direc+"/write"+str(x)+".txt"
        #fpd = open(fdbug,'w')
        #fpd.write("")
        #fpd.close()
        fpw = open(filew,'w')
...
def encode(filer):
    ...
    for character in content1:
        ...
        while node.character != character:
            if character in node.left_child.character:
                #print "test left " + node.left_child.character
                node = node.left_child
                bitstring += '0'
                continue
            #print "test right " + node.right_child.character
            node = node.right_child
            bitstring += '1'
...
def main():
    ...
    for character in cache:
        if flag:
            tablefile.write(character.encode("utf-8") + "\t" + cache[character])
            flag = False
        else:
